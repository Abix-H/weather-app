...
Each of the features is developed in a standalone microservice with its own Dockerfile, and the entire system is containerized with Docker and orchestrated with Docker Compose. The microservices communicate via HTTP and REST API, demonstrating key principles of cloud computing such as service decoupling, container-based deployment, and stateless inter-service communication.
...
We chose to build a weather-focused application because it naturally lends itself to modular responsibilities: fetching data from an external API, storing it, allowing users to retrieve it, and extending it with user authentication, notifications, and export functionality. Each of these capabilities was implemented as a separate microservice to ensure loose coupling and independent scalability.
...
| CSV Export Service   | 5003 | Retrieves weather history from the history service and generates downloadable CSV files.                                     |
| User Service         | 5002 | Handles user registration and login using hashed passwords. (Currently uses in-memory storage).                                |
...
To simplify deployment and ensure consistency across environments, the entire application is containerized using Docker and orchestrated using Docker Compose.
...
- Port mapping to expose services to localhost
...
- Start each container and attach logs

- Allow you to access the microservices at http://localhost:\<port>
